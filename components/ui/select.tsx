...
const selectTriggerVariants = cva(
  "flex h-9 w-full items-center justify-between gap-3 rounded-ele border border-border bg-background px-3 py-2 text-sm transition-all placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 border border-border bg-input",
  {
    variants: {
      variant: {
        default: "hover:bg-black hover:text-black-foreground shadow-sm/2",
        outline: "border-2 hover:border-ring shadow-sm/2",
        ghost:
          "border-transparent hover:bg-black hover:text-black-foreground",
      },
      size: {
...
>(({ className, children, icon: Icon, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative text-stone-300 flex w-full cursor-default select-none items-center rounded-lg py-2 pl-3 pr-8 text-sm outline-none focus:bg-stone-800 focus:text-white-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[disabled]:text-muted-foreground",
      className
    )}
    {...props}
  >
